...
SyntaxInformation[ PixelVaEAutoencoder ]= {"ArgumentsPattern"->{_,_,_}};


CZCreatePixelVaE[ inputUnits_, latentUnits_, h1_:500, h2_:500 ] := PixelVaEAutoencoder[
   inputUnits,
   latentUnits,
   NetGraph[{
...
Train[ PixelVaEAutoencoder[ inputUnits_, latentUnits_, net_ ] ][ examples_ ] := (
   f[assoc_] := MapThread[
   ...
   PixelVaEAutoencoder[ inputUnits, latentUnits, NetTrain[ net, f,
...
Sample[ PixelVaEAutoencoder[ _, _, net_ ] ] := Module[{s=ConstantArray[0,{28,28}],decoder=NetExtract[ net, "decoder" ],cond=NetExtract[ net, {"decoder","cond"}]},tmp=decoder;
